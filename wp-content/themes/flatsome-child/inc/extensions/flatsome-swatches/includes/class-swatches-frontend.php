	...
	public function swatch_html( $html, $term, $type, $args ) {
		...
		switch ( $type ) {
			...
			case 'ux_label':
				...
				$html  = sprintf(
					'<div class="%s ux-swatch--label %s" data-value="%s" data-name="%s" title="%s"><span class="ux-swatch__text">%s</span><span class="ux-swatch__price">%s <span class="woocommerce-Price-currencySymbol">₫</span></span></div>',
					implode( ' ', $classes ),
					$selected,
					esc_attr( $term->slug ),
					$name,
					esc_attr( $tooltip ),
					esc_attr( $term->name ),
					$price = number_format( esc_html( $label ), 0, ',', '.' )
				);
