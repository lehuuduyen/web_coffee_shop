 ...
 *
 * @wordpress-plugin
 * Plugin Name:       ZaloPay Payment Plugin
 * Plugin URI:        https://zalopay.vn/
 * Description:       ZaloPay Merchant integration plugin. (Plugin hỗ trợ thanh toán qua cổng thanh toán ZaloPay - Ví ZaloPay, ATM, CC, ApplePay & VietQR).
 * Version:           2.0.1 (Cập nhật 10/2023)
 * Author:            ZION Co.
...
const WC_ZALOPAY_VERSION = 'v2';
const ZLP_MIN_PHP_VER = '5.6.0';
const ZLP_MIN_WC_VER = '3.0';
const RETURNCODE_SUCCESS = 1;
const RETURNCODE_ERROR = -1;
const ZLP_QUERY_QUEUE = 'zlp_query_status';
const ZLP_CURRENCY = 'VND';
const ZLP_CREATE_ORDER_API = 'create';
const ZLP_QUERY_ORDER_STATUS_API = 'query';
const ZLP_REFUND_API = 'refund';
const ZLP_QUERY_REFUND_STATUS_API = 'query_refund';
	...
	if ( ! class_exists( 'Zlp' ) ) :
		...
		class Zlp {
			...
			public function init() {
				...
				require_once dirname( __FILE__ ) . '/includes/class-wc-zlp-vietqr.php';
				require_once dirname( __FILE__ ) . '/includes/class-wc-zlp-applepay.php';
			...
			public function add_gateways( $methods ) {
				...
				$methods[] = 'WC_Gateway_ZaloPay_VIETQR';
				$methods[] = 'WC_Gateway_ZaloPay_ApplePay';
				return $methods;
			}
...
function enqueue_custom_styles() {
	wp_enqueue_style('custom-plugin-styles', plugins_url('admin/css/zlp-admin.css', __FILE__));
}
