 ...
 */
class WC_Gateway_ZaloPay_ApplePay extends WC_Payment_Gateway
{
	public $appID;
	public $key1;
	public $key2;
	...
	{
		$this->id = 'zlp_applepay';
		$this->icon = plugins_url('assets/images/logo-apple-pay.png', dirname(__FILE__));
		$this->has_fields = false;
		$this->method_description = __('Cho phép khách hàng thanh toán qua Apple Pay. (Chú ý: Merchant phải có hỗ trợ thanh toán qua thẻ Credit card).', 'woocommerce-gateway-zalopay-atm');
		$this->supports           = array(
			'products',
            'refunds',
		);
		...
		// Load the settings.
		$this->init_settings();
		$this->title = __('ZaloPay Gateway (Apple Pay)', 'woocommerce-gateway-zalopay-applepay');
		$this->description = $this->gatewayAppleDescription ? $this->gatewayAppleDescription : $this->get_option('gatewayAppleDescription');
	...
	*/
	public function process_payment($orderID)
	{
		...
		if ($this->orderDescription != "") {
			...
			$desc = '[Tên đối tác] - Thanh toán đơn hàng #'.$orderID;
		}
		$args = [
