...
class Plugin {
	...
	public function enqueue_scripts() {
		...
		if ( is_checkout() ) {
			wp_enqueue_script( 'Ntvco-checkout-js' );
		}
	}
	...
	public function enqueue_admin_scripts() {
		...
		wp_register_script(
			'Ntvco-edit-order',
			NTVCO_ASSETS_URL . '/edit-order.js',
		...
		if ( $current_screen && 'shop_order' === $current_screen->id ) {
			wp_localize_script( 'Ntvco-edit-order', '_vnsOrderData', [
				'provinces' => array_values( Province::all() ),
			] );

			wp_enqueue_style( 'Ntvco-admin-css' );
			wp_enqueue_script( 'Ntvco-edit-order' );
		}
	}
	...
	public function get_asset_info( $name, $attribute = null ) {
		...
		$asset_path = untrailingslashit( NTVCO_PLUGIN_DIR_PATH ) . sprintf( '/dist/%s.asset.php', $name );

		if ( file_exists( $asset_path ) && is_readable( $asset_path ) ) {
			$info = $assets[ $name ] = include $asset_path;
		} else {
			$info = [ 'version' => NTVCO_VERSION, 'dependencies' => [] ];
		}
	...
	public function register_meta_box() {
		$renderCallback = function ( $post ) {
			...
			wp_add_inline_script(
				'Ntvco-edit-order',
				'window._NtvcoInitialStates = ' . wp_json_encode( $orderStates ),
				'before'
			);

			echo '<div id="NtvcoRoot"></div>';
		};

		add_meta_box(
			'NTVCO_box',
			esc_html__( 'Shipping', 'Ntvco' ),
			$renderCallback,
			'shop_order',
			'side',
			'high'
