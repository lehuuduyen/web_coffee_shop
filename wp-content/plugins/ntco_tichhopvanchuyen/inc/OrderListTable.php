...
use Ntvco\Courier\ShippingStatus;
use Ntvco\Traits\SingletonTrait;
...
class OrderListTable {
	...
	public function register_column( $columns ) {
		$add_column = [
			'NTVCO_info' => esc_html__( 'Shipping', 'Ntvco' ),
		];
	...
	public function eager_load_shipping_data( $posts, $wp_query ) {
		...
		$items = $wpdb->get_results(
			"SELECT * FROM `{$wpdb->prefix}NTVCO_data` WHERE `order_id` IN (" . implode( ',', $ids ) . ");"
		);
	...
	public function show_filters() {
		...
		$statuses = wp_list_pluck(
			$wpdb->get_results( "SELECT DISTINCT status FROM `{$wpdb->prefix}NTVCO_data`" ),
			'status'
		);

		?>
		<select name="_shipping_courier">
			<option value=""><?php echo esc_html__( 'Shipping courier', 'citygram' ); ?></option>
		...
		if ( ! empty( $statuses ) ) { ?>
			<select name="_shipping_status">
				<option value=""><?php echo esc_html__( 'Shipping status', 'citygram' ); ?></option>
	...
	public function query_posts_clauses( $clauses, $query ) {
		...
		if ( ! empty( $query->query_vars['_shipping_status'] ) || ! empty( $query->query_vars['_shipping_courier'] ) ) {
			if ( strpos( $clauses['join'], 'NTVCO_data' ) === false ) {
				$join_clause = " INNER JOIN `{$wpdb->prefix}NTVCO_data` NTVCO_data ON $wpdb->posts.ID = NTVCO_data.order_id ";

				if ( ! empty( $query->query_vars['_shipping_status'] ) ) {
					$join_clause .= $wpdb->prepare(
						' AND NTVCO_data.status = %s ',
