...
class ShippingController extends WP_REST_Controller {
	 ...
	 * @var string
	 */
	protected $namespace = 'Ntvco/ghn-aff';
		...
		register_rest_route(
			...
			[
				...
				'args' => [
					'shipping_method' => [
						'type' => 'string',
						'description' => __( 'An alphanumeric identifier for the courier.', 'Ntvco' ),
					],
				],
				'schema' => [ $this, 'get_public_item_schema' ],
			]
	...
	public function get_item_permissions_check( $request ) {
		if ( ! wc_rest_check_post_permissions( 'shop_order', 'read' ) ) {
			return new WP_Error( 'vn_shipping_rest_cannot_view',
				__( 'Sorry, you cannot view the resources.', 'Ntvco' ),
	...
	public function update_item_permissions_check( $request ) {
		if ( ! wc_rest_check_post_permissions( 'shop_order', 'edit', $request->get_param( 'order_id' ) ) ) {
			return new WP_Error(
				...
				__( 'Sorry, you are not allowed to update resources.', 'Ntvco' ),
				[ 'status' => rest_authorization_required_code() ]
			);
		}
	...
	public function get_shipping_data( WP_REST_Request $request ) {
		...
		if ( ! $order || 'shop_order' !== $order->get_type() ) {
			return new WP_Error(
				...
				esc_html__( 'Invalid order ID.', 'Ntvco' ),
				[ 'status' => 404 ]
			);
		}
	...
	public function cancel_shipment( $request ) {
		...
		if ( ! $shipping_data ) {
			return new WP_Error(
				'rest_invalid_order',
				esc_html__( 'Shipping code not created yet.', 'Ntvco' ),
				[ 'status' => 400 ]
			);
		}
	...
	protected function resolve_order( $request ) {
		...
		if ( ! $order || 'shop_order' !== $order->get_type() ) {
			throw new RuntimeException( esc_html__( 'Invalid order ID.', 'Ntvco' ), 404 );
		}
	...
	/**
	 * @param WP_REST_Request $request
	 * @return \Ntvco\Courier\AbstractCourier|mixed
	 */
	protected function resolve_courier( $request ) {
		...
		try {
			...
			throw new RuntimeException( esc_html__( 'Shipping courier is invalid', 'Ntvco' ), 400 );
		}
	}
	...
	protected function resolve_shipping_method( $request ) {
		...
		if ( ! $info || ! isset( $shipping_methods[ $info['id'] ] ) ) {
			throw new RuntimeException( esc_html__( 'Shipping method is invalid', 'Ntvco' ), 400 );
		}
