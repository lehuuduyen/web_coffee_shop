...
class GHNShippingMethod extends WC_Shipping_Method implements ShippingMethodInterface {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->method_title = __( 'Giao Hàng Nhanh', 'Ntvco' );
...
print_r( $ward) ;die('OK');*/
	...
	public function setup_setting_fields() {
		...
		$form_fields['api_token'] = [
			'type' => 'password',
			'title' => esc_html__( 'Token API', 'Ntvco' ),
		...
		$form_fields['is_debug'] = [
			'type' => 'checkbox',
			'title' => esc_html__( 'Debug mode', 'Ntvco' ),
			'label' => esc_html__( 'Is API under debug mode?', 'Ntvco' ),
			'default' => 'yes',
		];
		...
		if ( $this->get_option( 'api_token' ) ) {
			$form_fields['shop_id'] = [
				'type' => 'text',
				'title' => esc_html__( 'Store ID from GHN', 'Ntvco' ),
				'description' => esc_html__(
					'',
					'Ntvco'
				),
	...
	public function setup_instance_fields() {
		$this->instance_form_fields = [
			'title' => [
				...
				'default' => __( 'Giao Hàng Nhanh', 'Ntvco' ),
				'desc_tip' => true,
			],
		];
	}
