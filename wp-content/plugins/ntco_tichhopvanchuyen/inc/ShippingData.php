...
class ShippingData implements JsonSerializable {
	...
	public static function get( $order_id ) {
		...
		$_data = wp_cache_get( $order_id, 'NTVCO_data' );

		if ( ! $_data ) {
			$_data = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT * FROM `{$wpdb->prefix}NTVCO_data` WHERE `order_id` = %d LIMIT 1",
					$order_id
				)
			);
			...
			wp_cache_add( $_data->id, $_data, 'NTVCO_data' );
		}
	...
	public static function exists( $order_id ) {
		...
		return (bool) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(`id`) as count FROM `{$wpdb->prefix}NTVCO_data` WHERE `order_id` = %d LIMIT 1",
				absint( $order_id )
			)
		);
	}
	...
	public static function create( $order_id, array $data ) {
		...
		if ( $exists ) {
			$wpdb->update(
				$wpdb->prefix . 'NTVCO_data',
				$data,
				[ 'order_id' => $order_id ]
			);

			wp_cache_delete( $order_id, 'NTVCO_data' );
		...
		if ( false === $wpdb->insert( $wpdb->prefix . 'NTVCO_data', $data ) ) {
	...
	public function delete() {
		...
		if ( $deleted ) {
			wp_cache_delete( $this->order_id, 'NTVCO_data' );
		}
